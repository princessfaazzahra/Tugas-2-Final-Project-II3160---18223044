...
async function handleToggleStatus(catalogId, namaItem, currentStatus) {
    ...
    try {
        const response = await fetch(`${MICROSERVICE_URL}/makanan/${catalogId}/status`, {
            method: 'POST',
            headers: {
        ...
        await showAlert(`Gagal mengubah status makanan. Silakan coba lagi. Pesan: ${error.message}`, 'error');
    }
}
